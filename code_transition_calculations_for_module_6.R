########################################################################################
# Berechnung der Larson-Parameter für Modul 6, Prozessschritt PostWash
########################################################################################
...
overview_m6_resin <- readRDS("raw_data/overview_m6_resin.rds")

# res_m6_with_resin und dat_m6 müssen aus dem vorherigen Aufbereitungsskript vorhanden sein
...
first_bn <- overview_m6_resin$BatchNumber[1]

df_example <- res_m6_with_resin$data %>%
...
for (bn in overview_m6_resin$BatchNumber) {
  
  df_batch <- res_m6_with_resin$data %>%
...
pseudonyms <- paste0("M6_Batch", seq_len(nrow(df_sorted)))
...
write.csv(
  df_export,
  file = "data_for_modeling/m6_postwash_larson.csv",
  row.names = FALSE
)

write.csv(
  key_df,
  file = "pseudo_key/m6_key.csv",
  row.names = FALSE
)
